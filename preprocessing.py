...
class VAD():
    ...
    def get_audio(self, audio):       
        # Convert audio to a format suitable for processing
        resolution = tf.int16
        ...
        zero_padding = tf.zeros(16000 - tf.shape(audio_normalized), dtype=tf.float32)
        audio_padded = tf.concat([audio_normalized, zero_padding], axis=0)

        return audio_padded
    
    def is_silence(self, audio):
        # Determine if the audio contains silence based on energy threshold
